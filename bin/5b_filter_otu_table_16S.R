### Script to clean contaminant reads (negative controls) and
...
amptk = import_biom("../data/16S/amptk/taxonomy16S.biom")
amptk
...
run1 = subset_samples(amptk, Run == "T0152")
run2 = subset_samples(amptk, Run == "T0153")

run1a_nc = remove_nc_reads(phyloseq=amptk, batch_id = "T0152", NC_name = "Run1a.No")
otu_table(run1) = run1a_nc   ### replace OTU table with the filtered table

run2a_nc = remove_nc_reads(phyloseq=amptk, batch_id = "T0153", NC_name = "Run2a.No")
otu_table(run2) = run2a_nc

run2b_nc = remove_nc_reads(phyloseq=run2, batch_id = "T0153", NC_name = "Run2b.No")
otu_table(run2) = run2b_nc

run2c_nc = remove_nc_reads(phyloseq=run2, batch_id = "T0153", NC_name = "Run2c.No")
otu_table(run2) = run2c_nc
...
write.csv(otu_table(amptk_filtered), "../output/16S/OTU_table_16S.csv", row.names=TRUE)
write.csv(tax_table(amptk_filtered), "../output/16S/Taxonomy_table_16S.csv", row.names=TRUE)
